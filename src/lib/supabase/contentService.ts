...
export const createContentInSupabase = async (content: Omit<ContentItem, 'id' | 'created_at' | 'updated_at'>): Promise<ContentItem> => {
  const { data, error } = await supabase
    .from('contents')
    .insert([content])
    .select()
...
export const updateContentInSupabase = async (id: number, content: Partial<Omit<ContentItem, 'id' | 'created_at' | 'updated_at'>>): Promise<ContentItem> => {
  const { data, error } = await supabase
    .from('contents')
    .update(content)
    .eq('id', id)
...
export const subscribeToContentChanges = (callback: (payload: any) => void) => {
  return supabase
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'contents' }, callback)
    .subscribe();
};