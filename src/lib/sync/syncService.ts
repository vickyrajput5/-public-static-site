...
import { fetchSupabaseContent, createSupabaseContent, updateSupabaseContent, deleteSupabaseContent } from '../redux/supabaseContentSlice';
...
export const convertStrapiToSupabase = (strapiItem: StrapiContentItem): Omit<ContentItem, 'id' | 'created_at' | 'updated_at'> => {
  return {
...
export const convertSupabaseToStrapi = (supabaseItem: ContentItem): { title: string; description: string } => {
  return {
...
export const syncStrapiToSupabase = async (strapiItem: StrapiContentItem): Promise<void> => {
  try {
    // Check if the item already exists in Supabase
    const state = store.getState();
    const existingItem = state.supabaseContent.items.find(item => item.title === strapiItem.attributes.title);
    
    if (existingItem) {
      // Update existing item
      await store.dispatch(updateSupabaseContent({
        id: existingItem.id,
        content: convertStrapiToSupabase(strapiItem)
      }));
    } else {
      // Create new item
      await store.dispatch(createSupabaseContent(convertStrapiToSupabase(strapiItem)));
    }
  } catch (error) {
...
export const resolveConflict = (strapiItem: StrapiContentItem, supabaseItem: ContentItem): { title: string; description: string } => {
